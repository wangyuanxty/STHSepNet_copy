...
class ASTGCN_block(nn.Module):
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        self.ln = nn.LayerNorm(nb_time_filter)  # Put channel on the last dimension
    ...
    def forward(self, x):
        ...
        time_conv_output = self.time_conv(spatial_gcn.permute(0, 2, 1, 3))  # (b,N,F,T)->(b,F,N,T),Convolution kernel of (1,3)->(b,F,N,T)
        # residual shortcut
        x_residual = self.residual_conv(x.permute(0, 2, 1, 3))  # (b,N,F,T)->(b,F,N,T) Convolution kernel of (1,1)->(b,F,N,T)
