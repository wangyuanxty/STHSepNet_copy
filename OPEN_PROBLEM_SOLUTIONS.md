...
Gumbel-Softmax (ICLR 2017) 提供了离散采样的可微重参数化方法。GAT (ICLR 2018) 证明了可学习的连接权重比固定距离度量好——用评分网络替代欧氏距离 KNN，本质上是把"谁该连谁"这个决策从数学公式变成了可学习的函数。

---

## 5. 实验结果

BIKE-Inflow 数据集，BERT 主干，5 个 epoch（未跑满，趋势可用）：
...
方案二的 Test RMSE 从 14.40 降到了 13.61，降幅约 5.5%；MAE 从 5.18 降到 5.13，略有改善。方案一的 RMSE 降到了 13.86，降幅约 3.8%，但 MAE 反而略高——可能聚类数 C=8 不是最优值，需要调参。两个方案都只跑了 5 个 epoch，跑满 50 epoch 后效果应该更好。
